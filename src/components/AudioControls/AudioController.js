...
class AudioController {
  ...
  init (playlist, track = 0, onChangeStatus, onChangeCurrentTime) {
    ...
    this.load(this.currentAudio, isLoaded =>
      isLoaded
  ...
  play () {
    ...
    if (
      typeof this.currentAudio.currentTime !== 'undefined' &&
  ...
  seekToForward (seconds) {
    ...
    if (
      ...
      parseInt(this.currentAudio.duration, 10)
    )
      return
    ...
    this.currentTimeListener = setInterval(() => {
      this.getCurrentTime(seconds => {
        if (
          ...
          seconds > this.currentAudio.duration
        ) {
          this.player.pause()
          this.onChangeStatus(this.status.STOPPED)
          this.currentAudio.currentTime = 0
  ...
  startMusicControl () {
    ...
    MusicControl.setNowPlaying({
      ...
      artwork: this.currentAudio.thumbnailUri
        ? this.currentAudio.thumbnailUri
        : this.currentAudio.thumbnailLocal, //OK
